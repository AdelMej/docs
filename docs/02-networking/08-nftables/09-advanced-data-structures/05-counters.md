...
table ip filter {
    ...
    chain FORWARD {
        ...
        #ip saddr $ipvoipbox udp dport 5161 counter name voip2 comment "counting packets for SIP2"
        ip saddr $ipvoipbox counter name udp sport map @voipcounters
        ip saddr $ipvoipbox counter name udp dport map @voipcounters
    }
 }
```
...
Note: Resetting counters does not reset anonymous counters, see bug #1401.
