...
When a password is set for a group, members are permitted to join the group using `newgrp(1)` without providing a password, while non-members are required to supply the password [6]. If the command is called by a group administrator with only a group name, it prompts for a new group password.
