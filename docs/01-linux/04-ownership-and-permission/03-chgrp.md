...
The `chgrp` command changes the group of each file to a specified group [18]. With the `--reference` option, the group of each file is changed to that of a reference file (RFILE).

---

## Options Reference

The following options are available for the `chgrp` command [18]:
